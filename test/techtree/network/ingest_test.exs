defmodule Techtree.Network.IngestTest do
  ...
  describe "an address somebody volunteered" do
    ...
    test "is stored apart from the log, keyed by the participant key, never on the entry" do
      {:ok, entry, :recorded} =
    ...
    test "a second participant leaving the same address remains a separate record" do
      {:ok, first, :recorded} =
        NetworkFixture.publish(NetworkFixture.submission(), contributor_address: @address)
      ...
      records = Ingest.contributor_address(@address)
      assert length(records) == 2
      assert records |> Enum.map(& &1.participant_key_id) |> Enum.uniq() |> length() == 2
      assert first.participant_key_id != second.participant_key_id

      assert length(Network.list_publication_entries!()) == 2
    end

    test "spelled the other way retains the same unverified address" do
      NetworkFixture.publish(NetworkFixture.submission(), contributor_address: @address)
      ...
      assert Enum.all?(
               Ingest.contributor_address(@address),
               &(&1.contributor_address_unverified == String.downcase(@address))
             )
    end

    test "keeps separate records for separate participant keys" do
      NetworkFixture.publish(NetworkFixture.submission(), contributor_address: @address)
      NetworkFixture.publish(another_run(), contributor_address: @second_address)

      assert length(Ingest.contributor_address(@address)) == 1
      assert length(Ingest.contributor_address(@second_address)) == 1
    end
    ...
    test "removal means removal from the active system, however many runs supplied it" do
      ...
      assert length(Ingest.contributor_address(@address)) == 2

      assert :ok = Ingest.forget_contributor_address(String.downcase(@address))
      assert Ingest.contributor_address(@address) == []
      assert length(Network.list_publication_entries!()) == 2
    end
    ...
    test "cannot be reached through the domain by anything but the ingest" do
      ...
      assert {:error, _forbidden} = Network.find_contributor_addresses(String.downcase(@address))
    end
  end
