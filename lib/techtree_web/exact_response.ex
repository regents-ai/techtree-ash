defmodule TechtreeWeb.ExactResponse do
  ...
  Several endpoints return generated bytes verbatim, and all of them owe the
  caller the same things: the media type the catalog recorded, an entity tag that is
  ...
  # The run log is appended to and its entries can be marked withdrawn, so a
  # copy of one held anywhere may already be wrong.
  @no_store "no-store"

  @typedoc """
  How long a caller may keep the bytes: `:immutable` for content addresses,
  `:revalidated` for the documents a release replaces, `:no_store` for the log,
  which changes underneath a reader.
  """
  ...
  defp cache_control(:no_store), do: @no_store
