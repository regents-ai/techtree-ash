defmodule TechtreeWeb.SubmissionController do
  ...
  # What the network says back: where the entry landed, what was checked, and
  # the address the exact bytes now live at. The participant signed their run;
  # this is the site saying which checks it ran before accepting it.
  defp receipt(conn, entry, status) do
    ...
    body =
      Jason.encode_to_iodata!(%{
        ...
        "accepted_at" => entry.inserted_at,
        "checks_run" => entry.verification_checks_run,
        "checks_passed" => entry.verification_checks_passed,
        "location" => location
      })
