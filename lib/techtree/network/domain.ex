defmodule Techtree.Network do
  ...
  once a row is in it, it is in it: an entry is withdrawn by appending a
  withdrawal, never by deleting the row. Two concerns with different lifetimes,
  ...
  here because the ingest is what receives it, and nowhere else in this
  application may read it.
  """

  use Ash.Domain, otp_app: :techtree

  @default_maximum_body_bytes 4_000_000
  @default_rate_limit [limit: 10, window_seconds: 60]

  resources do
    resource Techtree.Network.Submission do
      define :list_submissions, action: :read
      define :list_published_submissions, action: :list_published
      define :get_submission_by_digest, action: :get_by_digest, args: [:bundle_digest]
      define :record_submission, action: :record
      define :mark_submission_withdrawn, action: :mark_withdrawn
    end

    resource Techtree.Network.Withdrawal do
      define :list_withdrawals, action: :read
      define :record_withdrawal, action: :record
    end

    resource Techtree.Network.ContributorAddress do
      define :get_contributor_address, action: :get_by_address, args: [:address]
  ...
  transcripts in it, so this is generous by a factor of ten and still small
  enough that a body over it can be refused before it is parsed.
  """
  @spec maximum_body_bytes() :: pos_integer()
  def maximum_body_bytes do
    :techtree
    |> Application.get_env(__MODULE__, [])
    |> Keyword.get(:maximum_body_bytes, @default_maximum_body_bytes)
  end
