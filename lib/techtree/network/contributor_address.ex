defmodule Techtree.Network.ContributorAddress do
  ...
  not about one run of theirs. In particular, two participant keys that type
  the same address remain two records; the address is not an identity and is
  never used to aggregate people.
  ...
  ## It is unverified, and the column says so in its own name
  ...
  ## Removal means removal from the active system and from any future use
  ...
  postgres do
    # Keep the legacy address-keyed table untouched. The corrected resource is
    # additive so an already-deployed empty table is never dropped in place.
    table "network_contributor_addresses_by_participant"
    repo Techtree.Repo
    identity_index_names unique_participant_key: "network_contributor_address_participant_key_idx"
  end
  ...
  actions do
    ...
    create :record do
      ...
      upsert? true
      upsert_identity :unique_participant_key
      upsert_fields [:contributor_address_unverified]
    end
  ...
  identities do
    identity :unique_participant_key, [:participant_key_id]
  end
end
